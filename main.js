let score = 0;
let highScore = localStorage.getItem('highScore') || 0; // Retrieve high score from localStorage or set to 0
...
class SnakeScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("background", "assets/pngwing.com.png");
  ...
  create() {
    ...
    this.scoreText = this.add.text(16, 16, `Score: ${score}`, { fontSize: '20px', fill: '#fff' });
    this.highScoreText = this.add.text(16, 40, `High Score: ${highScore}`, { fontSize: '20px', fill: '#fff' });
  }
  ...
  update(time) {
    ...
    if (this.snake.update(time)) {
      if (this.physics.overlap(this.snake.body[0], this.food)) {
        ...
        this.snake.grow();
        score++;
        this.scoreText.setText(`Score: ${score}`); // Update score on screen
      }
    }
  }
}
...
class Snake{
  ...
  endGame() {
    ...
    this.scene.highScoreText.setText(`High Score: ${highScore}`);
    score = 0;
  }
